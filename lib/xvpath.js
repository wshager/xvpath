...
function select(node) {
    ...
    while (paths.length > 0) {
        path = paths.shift();
        cur = (0, _xvseq._isSeq)(cur) && !(0, _xvnode._isNode)(cur) ? _mappedSelectImpl(cur, path) : selectImpl(cur, path);
    }
    cur._isNodeSeq = true;
    return cur;
}
...
function selectAttribute(node) {
    ...
    while (paths.length > 0) {
        path = paths.shift();
        cur = (0, _xvseq._isSeq)(cur) && !(0, _xvnode._isNode)(cur) ? _mappedSelectImpl(cur, path, true) : selectImpl(cur, path, true);
    }
    cur._isNodeSeq = true;
    return cur;
}
